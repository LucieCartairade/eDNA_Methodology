...
Need : 
    - barcode_list.txt : Is a list of barcode names (they will column name)
...
barcode_list = []
with open("barcode_list.txt",'r') as barcode_list_file:
    barcode_list = [line.strip() for line in barcode_list_file]

#print(barcode_list)
#barcode_list = [ "barcode" + f"{i:02}" for i in range(1, 25)]
...
with open("2nd_clust.fasta.clstr", "r") as file:
    ...
    # We have to add the last clust to the dict
    recluster_Dict[cluster_id] = clusters
...
def write_line(filout, line, nb_reads_Dict):
    ...
    with open("BLAST_out_reclustered_summary_tax_seq.txt", 'r') as filin:
        with open("BLAST_out_reclustered_summary_tax_seq_counts.txt", 'w') as filout:
            ...
            for line in filin:
                ...
                write_line(filout, line, nb_reads_Dict)
            print(compteur)
